...
class DelayedScheduler(torch.optim.lr_scheduler._LRScheduler):
    ...
    def __init__(
        ...
        self.current_step = 0
        self.cached_state = {
            r"self": self.state_dict(),
            r"scheduler": self.scheduler.state_dict(),
        }
        self.cached_state[r"self"][r"restarts"] = 1
        super(DelayedScheduler, self).__init__(
            scheduler.optimizer, scheduler.last_epoch, scheduler.verbose
        )
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            if key not in (r"scheduler", r"cached_state")
        }
    ...
    def reset(self) -> None:
        ...
        self.cached_state[r"scheduler"][r"base_lrs"] = [
            lr * self.gamma ** self.cached_state[r"self"][r"restarts"]
            for lr in self.cached_state[r"scheduler"][r"base_lrs"]
        ]
        self.cached_state[r"scheduler"][r"_last_lr"] = self.cached_state[r"scheduler"][
            r"base_lrs"
        ]
...
class Lookahead(torch.optim.Optimizer):
    ...
    def step(self, closure: Optional[Callable[[], Any]] = None) -> torch.Tensor:
        """
        ...
        for group in self.optimizer.param_groups:
            for p in group[r"params"]:
                ...
                if state[r"step"] >= self.total_steps:
                    ...
                    state[r"cached_params"].add_(
                        p.data - state[r"cached_params"], alpha=self.alpha
                    )
                    if self.synchronize_weights:
                        ...
                        if self.pullback_momentum == r"pullback":
                            ...
                            self.optimizer.state[p][
                                ...
                                1.0 - self.alpha, state[r"cached_momentum"]
                            )
                            state[r"cached_momentum"] = self.optimizer.state[p][
                                r"momentum_buffer"
                            ]
    ...
    def step(self, closure: Optional[Callable[[], Any]] = None) -> torch.Tensor:
        ...
        for group in self.param_groups:
            for p in group[r"params"]:
                ...
                if grad.is_sparse:
                    raise RuntimeError(
                        r"Lamb does not support sparse gradients, consider SparseAdam instead."
                    )
                ...
                if len(state) == 0:
                    assert all(
                        (
                            element not in state
                            for element in (r"state", r"exp_avg", r"exp_avg_sq")
                        )
                    )
                    state[r"step"] = 0
                ...
                step_size = (
                    group[r"lr"] * math.sqrt(bias_correction2) / bias_correction1
                )
