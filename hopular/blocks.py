...
class EmbeddingBlock(LightningModule):
    ...
    def __init__(
        ...
        self.__feature_boundaries = torch.cumsum(
            torch.as_tensor([0] + self.__input_sizes), dim=0
        )
        self.__feature_boundaries = (
            self.__feature_boundaries[:-1],
            self.__feature_boundaries[1:],
        )
        ...
        self.register_buffer(
            name=r"feature_types",
            tensor=torch.zeros(size=(len(input_sizes),), dtype=int),
        )
        ...
        self.register_buffer(
            name=r"feature_positions",
            tensor=torch.arange(len(self.__input_sizes), dtype=int),
        )
        self.feature_position_embeddings = torch.nn.Embedding(
            num_embeddings=len(self.feature_positions),
        ...
        self.feature_projections = torch.nn.ModuleList(
            [
                torch.nn.Sequential(
                    ...
                    torch.nn.LayerNorm(
                        normalized_shape=self.__feature_size,
    ...
    def reset_parameters(self) -> None:
        ...
        for module in self.feature_embeddings:
            for layer in filter(lambda _: hasattr(_, r"reset_parameters"), module):
                if type(layer) is torch.nn.Linear:
                    ...
                    bound = 1.0 / math.sqrt(
                        torch.nn.init._calculate_fan_in_and_fan_out(layer.weight)[0]
                    )
        ...
        if self.feature_type_embeddings is not None:
            torch.nn.init.kaiming_uniform_(
                self.feature_type_embeddings.weight, a=math.sqrt(5)
            )

        # Initialize feature-type-specific embeddings.
        if self.feature_position_embeddings is not None:
            torch.nn.init.kaiming_uniform_(
                self.feature_position_embeddings.weight, a=math.sqrt(5)
            )
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.feature_type_embeddings is not None:
            input_embedded = input_embedded + self.feature_type_embeddings(
                self.feature_types
            ).view(1, -1)
        if self.feature_position_embeddings is not None:
            input_embedded = input_embedded + self.feature_position_embeddings(
                self.feature_positions
            ).view(1, -1)
        input_embedded = input_embedded.view(
            input_embedded.shape[0], len(self.__input_sizes), self.__feature_size
        )
        return torch.cat(
            tuple(
                feature_projection(input_embedded[:, feature_index])
                for feature_index, feature_projection in enumerate(
...
class SummarizationBlock(LightningModule):
    ...
    def __init__(
        self, input_sizes: List[int], feature_size: int, dropout_probability: float
    ):
        """
        ...
        self.feature_summarizations = torch.nn.Sequential(
            ...
            torch.nn.Linear(
                in_features=self.__input_size,
                out_features=reduce(add, self.__input_sizes),
                bias=True,
            ),
        )
    ...
    def reset_parameters(self) -> None:
        ...
        for layer in filter(
            lambda _: hasattr(_, r"reset_parameters"), self.feature_summarizations
        ):
            if type(layer) is torch.nn.Linear:
                torch.nn.init.kaiming_uniform_(layer.weight, a=math.sqrt(5))
                bound = 1.0 / math.sqrt(
...
class HopfieldBlock(LightningModule):
    ...
    def __init__(
        ...
        assert (
            self.__num_features * feature_size
        ) == input_size, r"Invalid input/feature shapes specified."
    ...
    def reset_parameters(self) -> None:
        ...
        if self.hopfield_lookup.association_core.in_proj_weight is not None:
            torch.nn.init.kaiming_uniform_(
                self.hopfield_lookup.association_core.in_proj_weight, a=math.sqrt(5)
            )
        else:
        ...
        if self.hopfield_lookup.association_core.in_proj_bias is not None:
            ...
            bound = 1.0 / math.sqrt(
                torch.nn.init._calculate_fan_in_and_fan_out(
                    self.hopfield_lookup.association_core.q_proj_weight
                )[0]
            )
            torch.nn.init.uniform_(
                self.hopfield_lookup.association_core.in_proj_bias, -bound, bound
            )
        if not self.hopfield_lookup.association_core.disable_out_projection:
            torch.nn.init.kaiming_uniform_(
                self.hopfield_lookup.association_core.out_proj.weight, a=math.sqrt(5)
            )
            if self.hopfield_lookup.association_core.out_proj.bias is not None:
                bound = 1.0 / math.sqrt(
                    torch.nn.init._calculate_fan_in_and_fan_out(
                        self.hopfield_lookup.association_core.out_proj.weight
                    )[0]
                )
                torch.nn.init.uniform_(
                    self.hopfield_lookup.association_core.out_proj.bias, -bound, bound
                )
        for attribute in (r"k", r"v"):
            parameter_bias = getattr(
                self.hopfield_lookup.association_core, f"bias_{attribute}"
            )
            if parameter_bias is not None:
                parameter_weight = getattr(
                    self.hopfield_lookup.association_core, f"{attribute}_proj_weight"
                )
                bound = 1.0 / math.sqrt(
                    torch.nn.init._calculate_fan_in_and_fan_out(parameter_weight)[0]
                )
                torch.nn.init.uniform_(parameter_bias, -bound, bound)

    def forward(
        self, input: torch.Tensor, memory: torch.Tensor, memory_mask: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        retrieved_patterns = self.hopfield_lookup(
            (memory, input, memory), association_mask=memory_mask
        )
...
class HopularBlock(LightningModule):
    ...
    def __init__(
        ...
        self.__hidden_size_sample = (
            hidden_size
            if hidden_size > 0
            else max(1, self.__input_size // self.__num_heads)
        )
        self.__hidden_size_feature = (
            hidden_size
            if hidden_size > 0
            else max(1, self.__feature_size // self.__num_heads)
        )

        self.__scaling_factor = scaling_factor
        self.__dropout_probability = dropout_probability
        assert (
            self.__input_size % self.__feature_size == 0
        ), r"Invalid feature size specified!"
        ...
        self.sample_sample_associations = checkpoint_wrapper(
            HopfieldBlock(
                input_size=self.__input_size,
                feature_size=self.__feature_size,
                hidden_size=max(
                    1, int(self.__hidden_size_sample * self.__hidden_size_factor)
                ),
                num_heads=self.__num_heads,
        ...
        self.feature_feature_associations = checkpoint_wrapper(
            HopfieldBlock(
                input_size=self.__feature_size,
                feature_size=self.__feature_size,
                hidden_size=max(
                    1, int(self.__hidden_size_feature * self.__hidden_size_factor)
                ),
                num_heads=self.__num_heads,
                scaling_factor=self.__scaling_factor,
                dropout_probability=self.__dropout_probability,
    ...
    def forward(
        ...
        feature_memory_mask: torch.Tensor,
    ) -> torch.Tensor:
        """
        ...
        interactions = interactions.reshape(
            interactions.shape[1], -1, self.__feature_size
        )
...
class Hopular(LightningModule):
    ...
    def __init__(
        ...
        assert all(
            (input_size >= 1 for input_size in self.__input_sizes)
        ), r"Invalid input sizes specified!"
        assert (
            0.0 <= initial_feature_loss_weight <= 1.0
        ), r"Invalid initial feature loss weight!"
        assert (
            0.0 <= final_feature_loss_weight <= 1.0
        ), r"Invalid final feature loss weight!"
        ...
        assert (
            self.__warmup_steps + self.__cooldown_steps == num_steps_per_cycle
        ), r"Invalid training phases!"
        ...
        self.register_buffer(
            name=r"memory_ids",
            tensor=torch.arange(len(memory)) if memory_ids is None else memory_ids,
        )
        self.hopular_blocks = torch.nn.ModuleList(
            modules=[
                HopularBlock(
                    input_size=self.__input_size,
                    feature_size=self.__feature_size,
    ...
    def from_data_module(
        cls, data_module: DataModule, **kwargs: Dict[str, Any]
    ) -> r"Hopular":
        """
    ...
    def _reset_scheduler_state(self) -> None:
        ...
        for scheduler in (
            schedulers if hasattr(schedulers, r"__iter__") else [schedulers]
        ):
            if issubclass(type(scheduler), DelayedScheduler):
                ...
                if not self.__cold_restart:
                    ...
                    if hasattr(scheduler.scheduler, r"T_max"):
                        scheduler.scheduler.T_max = (
                            self.__warmup_steps + self.__cooldown_steps
                        ) - 1
    ...
    def _load_optimizer_cache(self) -> None:
        ...
        for optimizer in (
            optimizers if hasattr(optimizers, r"__iter__") else [optimizers]
        ):
    ...
    def _compute_performance(
        ...
        for feature_index, start, end in zip(
            range(len(self.__feature_boundaries[0])), *self.__feature_boundaries
        ):
            ...
            if feature_index in self.__feature_discrete:
                ...
                accuracy = (
                    (prediction.detach().argmax(dim=1) == target.detach().argmax(dim=1))
                    .float()
                    .mean()
                )
    ...
    def forward(
        self, input: torch.Tensor, memory_mask: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        Apply Hopular.
        ...
        if self.training and self.__memory_ratio < 1.0:
            memory_indices = torch.randperm(
                embeddings_memory.shape[1], device=embeddings_memory.device
            )
            memory_indices = memory_indices[
                : max(1, int(self.__memory_ratio * memory_indices.shape[0]))
            ]
            iteration_memory = iteration_memory[:, memory_indices]
            iteration_mask = (
                iteration_mask[:, memory_indices]
        ...
        for hopular_block in self.hopular_blocks:
            hopular_iteration = hopular_block(
                ...
                sample_memory_mask=iteration_mask,
    ...
    def training_step(
        self, batch: Tuple[torch.Tensor, ...], batch_index: int
        ...
        if performance_result.feature_count <= 0:
            assert (
                performance_result.target_count > 0
            ), r"No element for computing loss found!"
            feature_loss, feature_accuracy = 0.0, 0.0
            target_loss, target_accuracy = (
                performance_result.loss_target,
                performance_result.accuracy_target,
            )
            loss = target_loss
            annealing_factor = 0.0
        elif performance_result.target_count <= 0:
            assert (
                performance_result.feature_count > 0
            ), r"No element for computing loss found!"
            feature_loss, feature_accuracy = (
                performance_result.loss_feature,
                performance_result.accuracy_feature,
            )
            ...
            loss = (
                annealing_factor * feature_loss + (1.0 - annealing_factor) * target_loss
            )

        # Log weighted metrics.
        self.log_dict(
            {
                r"loss_feature/train": float(feature_loss),
        ...
        self.log_dict(
            {
                r"loss_target/train": float(target_loss),
                r"step": float(self.current_epoch),
            },
            on_step=False,
            on_epoch=True,
        ...
        if len(np.setdiff1d(self.__feature_discrete, self.__target_discrete)) > 0:
            self.log_dict(
                {
                    r"accuracy_feature/train": float(feature_accuracy),
                    r"step": float(self.current_epoch),
                },
                on_step=False,
                on_epoch=True,
        ...
        if len(self.__target_discrete) > 0:
            self.log_dict(
                {
                    r"accuracy_target/train": float(target_accuracy),
                    r"step": float(self.current_epoch),
                },
                on_step=False,
                on_epoch=True,
        ...
        self.log_dict(
            {
                r"annealing_factor": float(annealing_factor),
                r"step": float(self.current_epoch),
            },
            on_step=False,
            on_epoch=True,
        ...
        self.log_dict(
            {
                r"learning_rate": self.lr_schedulers().get_lr()[0],
                r"step": float(self.current_epoch),
            },
            on_step=False,
            on_epoch=True,
    ...
    def validation_step(
        ...
        if len(self.__target_discrete) > 0:
            ...
            self.log_dict(
                {
                    r"accuracy_target/val": float(accuracy),
                    r"step": float(self.current_epoch),
                },
                on_step=False,
                on_epoch=True,
    ...
    def test_step(
        ...
        if len(self.__target_discrete) > 0:
            self.log_dict(
                {
                    r"accuracy_target/test": float(accuracy),
                    r"step": float(self.current_epoch),
                },
                on_step=False,
                on_epoch=True,
    ...
    def on_train_epoch_end(self, **kwargs) -> None:
        ...
        if self._get_training_phase() == self.TrainingPhase.COOLDOWN:
            feature_loss_weight_range = (
                self.__initial_feature_loss_weight - self.__final_feature_loss_weight
            )
            ...
            self.annealing_factor += (
                0.5 * feature_loss_weight_range * (1.0 + math.cos(step_ratio * math.pi))
            )
            self.__annealing_step += 1
    ...
    def configure_optimizers(self) -> Tuple[List[Any], List[Any]]:
        ...
        learning_rate_scheduler = DelayedScheduler(
            scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer=optimizer,
                T_max=math.ceil(
                    (1.0 - self.__warmup_ratio)
                    * (self.__warmup_steps + self.__cooldown_steps)
                ),
                last_epoch=-1,
            ),
            first_step=math.floor(
                self.__warmup_ratio * (self.__warmup_steps + self.__cooldown_steps)
            ),
            gamma=self.__gamma,
        )
